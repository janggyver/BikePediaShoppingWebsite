...
if (!isset($_SESSION['cart'])) {	// Does the cart exist?  If not, create the session variables that represent it...
	...
	$_SESSION['ProductTotal'] = 0;	
	$_SESSION['GrandTotalPrice'] = 0;    // Declare the grand total price and initialize to zero.
	$_SESSION['cart'][$_SESSION['num_products']]['productsubtotal'] =0;
}
...
if (isset($_GET['product']) and isset($_GET['quantity'])) {
	...
	for ($i=0; $i<$_SESSION['num_products']; $i++) {
			...
			//Data Retrieve
	$strCartQuery = "SELECT ProductCode, ProductName, ProductDescription, RegularPrice, Department, Stock, ThumbnailHeight from products where ProductCode = '".$_SESSION['cart'][$i]
	['name']."'";
	...
	$rowCart = $rsCart->fetch_array(MYSQLI_ASSOC);
		...
		if ($_SESSION['cart'][$i]['name'] 	== 	$_GET['product']) {
			...
			$_SESSION['cart'][$i]['productsubtotal'] += $rowCart["RegularPrice"]*$_GET['quantity'];
			$_SESSION['ProductTotal'] =$_SESSION['cart'][$i]['productsubtotal'] ;
			$in_cart = true;
			}
	}
		$_SESSION['GrandTotalPrice'] += $_SESSION['ProductTotal'] ;	
	// If item is not in cart, add it
	if (!isset($in_cart)) {
		$_SESSION['cart'][$_SESSION['num_products']]['productsubtotal']=0;
		$_SESSION['cart'][$_SESSION['num_products']]['name'] 	= $_GET['product'];
		$_SESSION['cart'][$_SESSION['num_products']]['qty'] 	= $_GET['quantity'];
		$_SESSION['cart'][$_SESSION['num_products']]['productsubtotal'] += $rowCart["RegularPrice"]*$_GET['quantity'];
			
		$_SESSION['GrandTotalPrice'] +=	$_SESSION['cart'][$_SESSION['num_products']]['productsubtotal'] ;		
...
for ($i=0; $i<$_SESSION['num_products']; $i++) {
	...
	$strCartQuery = "SELECT ProductCode, ProductName, ProductDescription, RegularPrice, Department, Stock, ThumbnailHeight from products where ProductCode = '".$_SESSION['cart'][$i]['name']."'";
	$rsCart = $db_connected->query($strCartQuery);
	$rowCart = $rsCart->fetch_array(MYSQLI_ASSOC);
	...
	echo '<td>'.$rowCart["RegularPrice"]. '</td>'; // Regular Price
	...
	echo '<td>'.$_SESSION['cart'][$i]["productsubtotal"]. '</td>';  // Product Subtotal
 ...
 /*
 echo '<tr>
	<td>Product Sub Total Price: '.$_SESSION['cart'][$i]['productsubtotal'].'<br> Grand Total Price: '.$_SESSION['GrandTotalPrice'].'<br>';
echo '</tr>';
*/
...
echo '
		...
		<tr>
			<td> Grand Total </td> <td> </td> <td> </td> <td> </td><td> </td><td> '.$_SESSION['num_items'].'  </td><td> '.$_SESSION['GrandTotalPrice'].'  </td>
	</tr>
	<tr>
			<td> <hr> </td><td> <hr> </td><td> <hr> </td><td> <hr> </td><td> <hr> </td><td> <hr> </td><td> <hr> </td>
	</tr>
 ...
 *************************************************************************************************************************************/
   ...
   echo '<a href="javascript: history.go(-1)"><img src="../images/continueshopping.jpg">';
   echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="Checkout.php"><img src="../images/checkout.jpg"></a>';
}
else {
   echo '<br /><br /><a href="../index.php"><h2>Cart is empty now. Please pick happy goods first please. </h2><img src="../images/continueshopping2.jpg"></a>';
}
